...
while running:
    ...
    text_Erros = font_err.render("Error = " + str(int_errs), True, black)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if x_mouse >= rect_run_xy_A[0] and x_mouse <= rect_run_xy_B[0] and y_mouse >= rect_run_xy_A[1] and y_mouse <= rect_run_xy_C[1]:
                ...
                labels = []                
                if clusters == []: # Xử lý lỗi khi chưa nhấn random, đã chạy run
                    continue
            ...
            if x_mouse >= rect_Algorithm_xy_A[0] and x_mouse <= rect_Algorithm_xy_B[0] and y_mouse >= rect_Algorithm_xy_A[1] and y_mouse <= rect_Algorithm_xy_C[1]:
                ...
                # Huấn luyện points theo clusters: fit như trainner là huấn luyện                
                if K>0:
                    kmeans = KMeans(n_clusters= K).fit(points)

                # Dự báo các point cùng nhu cầu clusters
                labels = kmeans.predict(points) # predict: dự đoán

                # Gán lại vị trí mới cho clusters
                clusters = kmeans.cluster_centers_                

                "Ghi chú: Trong Meachenlearnig: fit: huấn luyện máy học, predict: dự đoán dữ liệu mới"
                "Single prediction: dự đoán 1 kết quả, ngược lại batch prediction"
    ...
    for i_clus, cluster in enumerate(clusters):
        ...
        pygame.draw.circle(screen, black, cen, 20)
        pygame.draw.circle(screen, colors[i_clus], cen, 16)
    ...
    # Tính tổng khoảng cách cùng nhóm đến cluster mới gán vào biến erros
    if clusters != [] and labels != []:
        int_errs = 0
        for i_point, point in enumerate(points):
            dis_cluster = fun_distance(point, clusters[labels[i_point]])
            int_errs += int(dis_cluster)
